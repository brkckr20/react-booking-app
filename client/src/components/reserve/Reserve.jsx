...
const Reserve = ({ setOpen, hotelId }) => {
    ...
    const isAvaliable = (roomNumber) => {
        const isFound = roomNumber.unavaliableDates.some((date) => alldates.includes(new Date(date).getTime()))
        return !isFound
    }
    ...
    const handleClick = async () => {
        try {
            await Promise.all(selectedRooms.map(roomId => {
                const res = axios.put(`${process.env.REACT_APP_API_URL}/rooms/avaliability/${roomId}`, { dates: alldates });
                return res.data
            }))
            setOpen(false);
            navigate("/")
